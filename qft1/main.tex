...
\begin{align}
    [S_z, a^\dagger_{0s}]\ket{0} &= \int \frac{d^3p}{(2\pi)^3}\frac{1}{2E_p}\sum_{r s'}(2\pi)^3 \delta_{ss'}\delta^{(3)}(\vec{p})u^{s'\dagger}(p)\frac{\Sigma^3}{2}u'^{r}(p)\ket{0} \\
    &= \frac{1}{2m}\sum_r u^{s\dagger}(0)\frac{\Sigma^3}{2}u^r(0)a^\dagger_{0r}\ket{0}
\end{align}

To proceed, write explicitly the spinors: 

\begin{align}
    u^s(p=0) = \sqrt{m}\begin{pmatrix}
        \xi \\ \xi 
    \end{pmatrix} \implies [S_z, a^\dagger_{0s}]\ket{0} = \frac{1}{2}\sum_r \begin{pmatrix}
        \xi^{r\dagger} & \xi^{r\dagger}
    \end{pmatrix} \frac{\Sigma^3}{2}\begin{pmatrix}
        \xi^r  \\ \xi^r
    \end{pmatrix}a^\dagger_{0s}\ket{0}
\end{align}

But remember that the 2-dimensional spinors are 

\begin{align}
    \xi^{r=1} = \begin{pmatrix}
        1 \\ 0
    \end{pmatrix}, \quad \xi^{r=2} = \begin{pmatrix}
        0 \\ 1 
    \end{pmatrix}
\end{align}

So the eigenvalues are \(\pm 1/2\), if \(s = 1, 2\).  

Lastly, remember that the lagrangian has a \(U(1)\) global symmetry. The charge operator, after applying normal ordering, looks like

\begin{align}
    Q = \int \frac{d^3p}{(2\pi)^3}\sum_{s_1, s_2}(a^\dagger_{ps}a_{ps} - b^\dagger_{ps}b_{ps})
\end{align}

so particles (created by \(a^\dagger\)) carry a positive charge, while antiparticles, created by \(b^\dagger\) have a negative charge. 
...
Since now, we've only considered non-interacting theories. Since we'd like to use this theory to make predictions about real events, we have to start to take into consideration what happens when we have interaction between fields. 

The main problem when one introduces interaction, is that normally the equations of motion become non linear: thus we cannot expand the solutions in terms of plane waves anymore. To be more specific, we can pick a preferred time, and there expand via fourier analysis our fields: but then this will evolve in time through a non-linear Hamiltionian.

For example, a one particle state \(a^\dagger_k\ket{0}\) can evolve into \((a^\dagger_k)^2\ket{0}\), i.e. a two particles state. This means that the Hilbert space of the interacting theory is going to be very different from the hilbert space of the free theories. 

We're going to assume that the interactions are \emph{local}, and that they cointain only the fields, not their derivatives. The hamiltonian of the interaction is going to be labeled 

\begin{align}
    H_{int} = \int d^3x \mathcal{H}_{int} = -\int d^3x \mathcal{L}_{int}
\end{align}

Some popular interaction theories are given by 

\begin{enumerate}
    \item \(\phi^4\) theory (auto-interacting scalar field): 
    \begin{align}
        \mathcal{L} = \frac{1}{2}(\partial_\mu \phi)^2 - \frac{1}{2}m^2\phi^2 - \frac{\lambda}{4!}\phi^4
    \end{align}
    \item quantum electrodynamics (interaction between spinor and gauge \(U(1)\) field): 
    \begin{align}
        \mathcal{L}_{QED} = \bar{\psi}(i\slashed{\partial}-m)\psi - \frac{1}{4}F_{\mu\nu}F^{\mu\nu} - e\bar{\psi}\gamma^\mu A_\mu \psi 
    \end{align}
    \item Yukawa coupling (interaction between scalar and spinor field): 
    \begin{align}
        \mathcal{L}_{YK} = \bar{\psi}(i\slashed{\partial}-m)\psi + \frac{1}{2}(\partial_\mu \phi)^2 - \frac{1}{2}m^2 \phi^2 -g\bar{\psi}\psi \phi^2
    \end{align}
\end{enumerate}


What we're mainly interested in computing in QFT are scattering amplitudes, i.e. probabilities that a certain interaction will occur. An exact computation of this processes can be done only in \(1+1\) dimensions. In a more realistic scenario, one must compute the amplitudes using perturbation theory. 

So far, we've only worked in the Heisenberg picture. For a moment, let's get back to the Schroedinger picture, and consider a state \(\ket{a}\) ad a fixed time \(T\). Of course, this state will evolve in time, but at the beggining, it will be an eigenstate of a set of operators, that we label by \(a\). Similarly, we also consider a different state \(\ket{b}\). If we let the states evolve with time, we get 

\begin{align}
    \ket{a(t)} = e^{-iH (t-T)}\ket{a}
\end{align}

and the matrix element of the transition \(\ket{a} \rightarrow \ket{b}\) at a time \(T_f\) will be 

\begin{align}
    \bra{b}e^{-iH(T_f-T)}\ket{a}
\end{align}

We define the \emph{S matrix} as the matrix whose element are given by the latter amplitude, as the difference in time becomes infinite: 

\begin{align}
    \bra{b}S\ket{a} := \lim_{(T_f-T) \rightarrow \infty} \bra{b}e^{-iH(T_f-T)}\ket{a}
\end{align}

\clm{}{}{
    From probability conservation, \(SS^\dagger = 1\), so \(S\) is \emph{unitary}. 

}

To see this, define a complet set of states \(\ket{n}\) such that \(\sum_n \ket{n}\bra{n} = 1\). Now, since the set is complete, the probability that \(\ket{a}\) will transition into one of this is 1, in formulas 

\begin{align}
    \sum_n |\bra{n}S\ket{a}|^2 = 1 \implies \sum_n \bra{a}S^\dagger \ket{n} \bra{n}S\ket{a} = 1 \implies \bra{a}SS^\dagger\ket{a} = 1
\end{align}

And if the starting state is correctly normalized, \(S\) must be unitary. 

Many times it's useful to write

\begin{align}
    S = \mathbb{I} + i T
\end{align}

that is, separate the propability that the state will remain the same, or having particle interaction. 

Now let's get back to the Heisenberg picture: remembering how the two pictures are related, the matrix elements of the S matrix are quickly found: 

\begin{align}
    \bra{b}S\ket{a} = \Braket{b, T_f | a, T_i}
\end{align}

Now, it's clear that we'd like a way of computing the \(S\) matrix. One way is called the LSZ formula. Consider then the matrix element between two states, of \(n\) and \(m\) particles, so \(\Braket{\vec{p}_1, \ldots, \vec{p}_n, T_f | \vec{k}_1, \ldots \vec{k}_n, T_i}\). Imagine that this particle state are created by a real scalar free field. We can invert the expansion of the field to obtain the ladder operators: 

\begin{align}
    &\sqrt{2E_k}a_k = i \int d^3x e^{ikx} \overleftrightarrow{\partial}_0 \phi_{free} \\
    &\sqrt{2E_k}a^\dagger_k = -i\int d^3x e^{-ikx}\overleftrightarrow{\partial}_0 \phi_{free}
\end{align}

If \(T_f-T_i \rightarrow \infty\), particles are well separated. Assuming the interaction decreases in magnitude with distance, we can imagine that in that limit, the relevant theory is the free theory. We then write

\begin{align}
    \phi(x) \rightarrow \begin{cases}
        \sqrt{Z}\phi_{in}(x), \quad \text{as } t \rightarrow -\infty \\
        \sqrt{Z}\phi_{out}(x), \quad \text{as } t \rightarrow \infty 
    \end{cases}
\end{align}

with \(Z\) a normalizing factor. Thus, we have 

\begin{align}
    \sqrt{2E_k}a^{\dagger(in)}_k &= -i\int d^3x e^{-ikx}\overleftrightarrow{\partial}_0 \phi_{in} \\
    &= -i\sqrt{Z}\lim_{t\rightarrow -\infty}\int d^3x e^{-ikx}\overleftrightarrow{\partial}_0 \phi(x) 
\end{align}

and in the same way, 

\begin{align}
    \sqrt{2E_k} = i\sqrt{Z}\lim_{t\rightarrow \infty}\int d^3x e^{-ikx}\overleftrightarrow{\partial}_0 \phi(x) 
\end{align}

Notice that the two operators act on different spaces: the initial/final states at \(T_i / T_f\). 

This gives a way to compute the S matrix element: 

\begin{align}
    \Braket{\vec{p}_1, \ldots, \vec{p}_n, T_f | \vec{k}_1, \ldots \vec{k}_m, T_i} &= \sqrt{2E_{k_1}}\bra{\vec{p}_1, \ldots, \vec{p}_m, T_f}a^{\dagger(in)}_{k_1}\ket{\vec{k}_2, \ldots, \vec{k}_m, T_i} \\
    &= -iZ^{-1/2}\lim_{t\rightarrow -\infty}\int d^3x e^{-ik_1x}\bra{\vec{p}_1, \ldots \vec{p}_n, T_f}\overleftrightarrow{\partial}_0\phi\ket{\vec{k}_2, \ldots, \vec{k}_m, T_i}
\end{align}

Now this is good but doesn't quite look lorentz covariant. We can write it in a way that makes it more explicit, using the fondamental theorem of calculus: 

\begin{align}
    \left(\lim_{t\rightarrow \infty} - \lim_{t\rightarrow -\infty} \right)f(t) = \int_{-\infty}^{+\infty}dt \frac{df}{dt} \implies \sqrt{2E_{k}}(a^{\dagger(in)}_k - a^{\dagger(out)}_k) = \int_{-\infty}^{+\infty} \int d^3x (iZ^{1/2})\partial_0 (e^{-ikx}\overleftrightarrow{\partial}_0 \phi)
\end{align}

Applying the derivative and remembering that \(\partial_0^2 e^{-ikx} = -k_0^2 e^{-ikx} = (\nabla^2 -m^2)e^{-ikx}\), we write 

\begin{align}
    \sqrt{2E_{k}}(a^{\dagger(in)}_k - a^{\dagger(out)}_k) = i Z^{1/2}\int d^4x (e^{-ikx}\partial_0^2\phi - \phi(\nabla^2-m^2)e^{ikx})
\end{align}

Now, we want to work with quantities that are localized in space: we can then integrate by parts in space, and drop the boundary terms, to get 

\begin{align}
    \sqrt{2E_{k}}(a^{\dagger(in)}_k - a^{\dagger(out)}_k) = iZ^{1/2}\int d^4x e^{-ikx}(\square +m^2)\phi(x)
\end{align}

please note that the integral is \emph{not} zero, as the field doesn't satisfy the KG equation everywhere in spacetime, but only at some fixed times. 

Using this useful relation, we get back to our matrix elements and we write 

\begin{align}
    \sqrt{2E_{k_1}}\bra{\vec{p}_1, \ldots, \vec{p}_n, T_f}(a^{\dagger(in)}_{k_1}- a^{\dagger(out)}_{k_1})\ket{\vec{k}_2, \ldots, \vec{k}_m, T_i} = \\
    = i Z^{1/2}\int d^4x e^{-ikx}(\square +m^2)\bra{\vec{p}_1, \ldots, \vec{p}_n, T_f}\phi(x)\ket{\vec{k}_2, \ldots, \vec{k}_m, T_i}
\end{align}

this is exactly the matrix element from the beggining, as \(a^{\dagger(out)}_{k_1}\) acts on the bra annihilating it. 

Ultimately, we'd like to use this trick to eliminate all the states. We have to understand how to eliminate the final states, then. To do this we start with 

\begin{align}
    \bra{\vec{p}_1, \ldots, \vec{p}_n, T_f}\phi(x)\ket{\vec{k}_2, \ldots, \vec{k}_m, T_i}
\end{align}

and we try to remove the \(\vec{p}_1\) particle, with the \(a_{p_1}^{(out)}\) operator. So we get 

\begin{align}
    \bra{\vec{p}_1, \ldots, \vec{p}_n, T_f}\phi(x)\ket{\vec{k}_2, \ldots, \vec{k}_m, T_i} = \sqrt{2E_{p_1}}\bra{\vec{p}_2, \ldots, \vec{p}_n, T_f}a_{p_1}^{(out)}\phi(x)\ket{\vec{k}_2, \ldots, \vec{k}_m, T_i}
\end{align}

we define the \emph{time ordered} product: 

\begin{align}
    T\left\{\phi(x) \phi(y) \right\} = \begin{cases}
        \phi(y)\phi(x) \quad \text{if } x^0 < y^0 \\
        \phi(x)\phi(y) \quad \text{if } x^0 > y^0 
    \end{cases}
\end{align}

Since \(a_p^{(out)}\) is defined as \(y^0 \rightarrow \infty\), it goes always first in the time product. Similarly \(a^{(in)}\) goes always last. We notice than that 

\begin{align}
    \sqrt{2E_{p_1}}\bra{\vec{p}_2, \ldots, \vec{p}_n, T_f}T\left\{(a_{p_1}^{(out)}-a_{p_1}^{(in)})\phi(x)\right\}\ket{\vec{k}_2, \ldots, \vec{k}_m, T_i} = \\
    = \sqrt{2E_{p_1}}\bra{\vec{p}_2, \ldots, \vec{p}_n, T_f}a_{p_1}^{(out)}\phi(x)\ket{\vec{k}_2, \ldots, \vec{k}_m, T_i}
\end{align}

Here we see why we had to introduce the time ordering product: since \(T(a_p^{(in)}\phi(x)) = \phi(x)a_p^{(in)}\), the distruction operator can act on the initial state and annihilate it. As before we get 

\begin{align}
    \sqrt{2E_{p_1}}(a^{(out)}_{p_1} -a^{(in)}_{p_1}) = -iZ^{-1/2}\int d^4x e^{ip_1 y}(\square +m^2)\phi(y)
\end{align}

in the end, we get 

\begin{align}
    \bra{\vec{p}_1, \ldots, \vec{p}_n, T_f}\phi(x)\ket{\vec{k}_2, \ldots, \vec{k}_m, T_i} = iZ^{1/2}\int d^4y e^{ip_1y}(\square +m^2)\bra{\vec{p}_2, \ldots, \vec{p}_n, T_f}T\left\{\phi(y)\phi(x)\right\}\ket{\vec{k}_2, \ldots, \vec{k}_m, T_i}
\end{align}

This procedure can now be easily iterated, and we find 

\begin{align}
    &\Braket{\vec{p}_1, \ldots, \vec{p}_n, T_f | \vec{k}_1, \ldots , \vec{k}_m, T_i} = \\
    &= \int \prod_i^m d^4x^i \prod_j^n d^4y^j \exp{i\sum_j^n p_jy_j - i\sum_i^m k_i x_i}(\square_{x_1}+m^2)\ldots(\square_{y_1} + m^2)\ldots \bra{\Omega}T\left\{\phi(x_1)\ldots\phi(x_n) \right\}\ket{\Omega}
\end{align}

where with \(\ket{\Omega}\) we've labeled the vacuum of the interacting theory. In the Schroedinger's picture, the last equation tells us 

\begin{align}
    \bra{\vec{p}_1, \ldots, \vec{p}_n}iT\ket{\vec{k}_1, \ldots, \vec{k}_m}
\end{align}

since the states are supposed to be different from one another. We understand then that the quantity 

\begin{align}
    \bra{\Omega}T\left\{\phi(x_1)\ldots \phi(x_n) \right\}\ket{\Omega}
\end{align}

this objects are called \emph{green functions} (for reason that'll become apparent later on), or time-ordered correlation functions. Tipicalli one deals with this functions under fourier transform: this is the natural thing to do, as we've been labelling all the states by their momenta. Another reason why this is useful is that the action of the KG operators becomes straightforward: 

\begin{align}
    G(x_1, \ldots, x_n) = \int \prod_i^n \frac{d^4k^i}{(2\pi)^4}e^{-i\sum_i k_ix_i}\tilde{G}(k_1, \ldots k_n) \implies (\square_{x_j}+m^2)G = -\int \prod_i^n \frac{d^4k_i}{(2\pi)^4}(k_j^2-m^2)e^{-i\sum_ik_ix_i}\tilde{G}
\end{align}

Putting together what we've learned, we get the famous LSZ formula: 

\begin{align}
    \int \prod_i^m d^4x^i e^{-ik_ix_i}\int \prod_j^m d^4y^j e^{ip_jy_j} \bra{\Omega}T\left\{\phi(x_1)\ldots \phi(x_n) \right\}\ket{\Omega} = \\
    \left(\prod_i^m \frac{i\sqrt{Z}}{k_i^2-m^2} \right)\left(\prod_j^n \frac{i\sqrt{Z}}{p_j^2-m^2} \right)\bra{\vec{p}_1, \ldots, \vec{p}_n}iT\ket{\vec{k}_1, \ldots \vec{k}_m}
\end{align}

Notice that the RHS is singular when the momenta are on-shell. Of course, this means that the green's functions (that appear on the LHS) are singular too. In practice, we can use the RHS to read the residues of the poles of the green functions. 

\section{Perturbative expansion of corelators}


The time product in the LSZ formula is expressed in terms of the interacting fields. This is not very useful in practice. Instead, we can switch to the interaction picture. Avoiding for the moment the details of the procedure, one finds in the end 

\begin{align}
    \bra{\Omega}T\left\{\phi(x_1)\ldots \phi(x_N)\right\}\ket{\Omega} = \frac{\bra{0}T\left\{\phi_I(x_1)\ldots \phi_I(x_N)e^{-i\int d^4x \mathcal{H}_I}\right\}\ket{0}}{\bra{0}T\left\{e^{-i\int d^4x \mathcal{H}_I}\right\}\ket{0}}
\end{align}

where \(\phi_I\) in this equations are the interaction picture fields. Still, we have to understand, or develop a procedure, to compute this time ordered products. 

To get an idea on how we might tackle this problem, we look at a very meaningful example: 


\ex{The Feynman propagator}{

    Consider the vaccuum expectaction value of the time ordered product of two fields, 

    \begin{align}
        \bra{0}T\left\{\phi_I(x)\phi_I (y) \right\}\ket{0}
    \end{align}

    this is called a Feynman propagator, or sometimes two point function (Also the green function, as it turns out that it's also the green function of the KG equation). We are going to compute this for a real scalar field. 

    Write 

    \begin{align}
        \phi = \phi^+ + \phi^-, \quad \begin{cases}
            \phi^+ = \int \frac{d^3p}{(2\pi)^3}\frac{1}{\sqrt{2E_p}}a_p e^{-ipx} \\
            \phi^- = \int \frac{d^3p}{(2\pi)^3}\frac{1}{\sqrt{2E_p}}a^\dagger_p e^{ipx}
        \end{cases}
    \end{align}

    This is useful as \(\phi^+\ket{0} = \bra{0}\phi^- = 0\). 

    For now, assume \(x^0 > y^0\): 

    \begin{align}
        T\left\{\phi(x)\phi(y) \right\} = \phi(x)\phi(y) = \phi^+(x)\phi^+(y) + \phi^+(x)\phi^-(y) + \phi^-(x)\phi^+(y) + \phi^-(x)\phi^-(y) 
    \end{align}

    It would be very useful if we maneged to write this in terms of a normal ordered term, plus some rest. This is because we are going look at the expectation value of this operator on the vaccum state. We see that the expression above is almost time ordered, except for 

    \begin{align}
        \phi^-(x)\phi^+(y) = \phi^+(y)\phi^-(x) + [\phi^-(x), \phi^+(y)] \implies T\left\{\phi(x)\phi(y)\right\} = :\phi(x)\phi(y): + [\phi^+(x), \phi^-(y)] 
    \end{align}

    A similar computation can be performed if \(x^0 < y^0\) (Just swap \(x \leftrightarrow y\)). In the end one finds that, in general, 

    \begin{align}
        T\left\{\phi(x)\phi(y)\right\} = :\phi(x)\phi(y): + \theta(x^0-y^0)[\phi^+(x), \phi^-(y)] + \theta(y^0-x^0)[\phi^+(y), \phi^-(x)]
    \end{align}
    ...
    \begin{align}
        ...
        = \theta(x^0-y^0)[\phi^+(x), \phi^-(y)] + \theta(y^0-x^0)[\phi^+(y), \phi^-(x)] = D(x-y)
    \end{align}

    As it's clear that the commutator of the two fields is a number (not an operator anymore), and we can assume the vaccum to be normalized. 

    Writing explicitly the two fields, and using the commutation relation \([a_p, a^\dagger_{p'}] = (2\pi)^3\delta(p-p')\), we quickly find 

    \begin{align}
        D(x-y) = \int \frac{d^3 p}{(2\pi)^3}\frac{1}{2E_p}\left( \theta(x^0-y^0)e^{-ip(x-y)} + \theta(y^0-x^0)e^{ip(x-y)}\right)
    \end{align}

    The expression is more elegant if we put it in it's covariant form: 

    \begin{align}
        D(x-y) = \int \frac{d^4p}{(2\pi)^4}\frac{ie^{-ip(x-y)}}{p^2-m^2 +i\eps}
    \end{align}

    The equivalence of the two can be found starting from the last one, separating time and spatial components of the momentum, and than compute the time integral using the residue theorem. 
}

\subsection{Wick's theorem}

We've seen that the computation of the 2-point function becomes straightforward if we convert the time ordered operator into a normal ordered expression plus a rest. How does this generalize when we want to compute the n-point functions? 

The answer is given by Wick's theorem: 

\begin{align}
    T\left\{\phi(x_1)\ldots \phi(x_N) \right\} = :\phi(x_1)\ldots \phi(x_N): + \left(\text{All possible combinations of time ordering and contractions} \right) 
\end{align}

Where by contraction of two fields we mean the Feynman propagator between the two, \(D(x_i-x_j)\). 

Wick's theorem can be proven by induction for every \(N\). Of course, we've already proved the base of induction, in computing the 2-point function. Since all of this might start to fell a bit abstract, we will now perform a simple computation, of the matrix element of a process using both the LSZ formula, and Wick's theorem. 


\ex{2 -> 2 scattering amplitues in \(\phi^4\) theory}{
    ...
    this describes an auto-interacting scalar field, with a 4-vertex. What does this mean? Let's consider the following process: 

    \begin{align}
        \phi(p_1) + \phi(p_2) \rightarrow \phi(k_1) + \phi(k_2)
    \end{align}

    write explicitly the LSZ formula: 

    \begin{align}
        \int d^4x_1 d^4x_2 d^4y_1 d^4y_2 e^{i(p_1x_1-p_2x_2-k_1y_1-k_2y_2)} \bra{\Omega}T\left\{\phi(x_1)\phi(x_2)\phi(y_1)\phi(y_2) \right\}\ket{\Omega}= \\
        = \left(\frac{1}{p_1^2 - m^2} \right)\left(\frac{1}{p_2^2 - m^2} \right)\left(\frac{1}{k_1^2 - m^2} \right)\left(\frac{1}{k_2^2 - m^2} \right)\bra{p_1, p_2}iT\ket{k_1k_2}
    \end{align}
...
\chapter{The path integral formulation of QFT}

Until now, we've only worked in the canonical quantisation. We've also come pretty far with it, as we maneged to describe free theories, interacting theories (altough only perturbatively), computing also cross-sections. Why do we want to go back to the roots and introduce another quantisation procedure? There is more than one reason: 

\begin{enumerate}
    \item some theories have strong coupling (e.g. QCD at hadronic energies), and cannot be described using perturbation theory. In the path integral formalism, one can treat interacting fields exactly. 
    \item we've seen that gauge invariance led to some problems when quantizing a gauge theory, even in the abelian case (electrodynamics). This gets worse if one considers a non-abelian gauge theory, and path integral formalism can avoid this problem. 
    \item The formalism highlights a profund connection between QFT and statistical mechanics. 
\end{enumerate}

\section{Formalism in standard QM}

In the Schroedinger picture, the amplitude of a state \(\ket{q_i}\) going into a state \(\ket{q_f}\) after a time \(\Delta T\) is given by 

\begin{align}
    A = \bra{q_f}e^{-iH\Delta T}\ket{q_i}
\end{align}

Instead, in the Heisenberg picture we have 

\begin{align}
    A = \Braket{q_f, T_f| q_i, T_i}
\end{align}

note that in the Heisenberg picture the states are time indipendent, and the operators are the objects that evolve with time. Still, we denote a state with a time label, to indicate that at that time, the state is known to be the eigenstate of a certain set of operators, with some eigenvalues. 

At every time, the states \(\ket{q_j, T_j}\) form a complete set, so we could write 

\begin{align}
    A = \int dq_j \Braket{q_f, T_f | q_j T_j}\Braket{q_j T_j | q_i T_i}
\end{align}

But also 

\begin{align}
    A = \int dq_1\ldots dq_{N-1}\Braket{q_f, T_f | q_1 T_1}\ldots \Braket{q_{N-1} T_{N-1} | q_f T_f}
\end{align}

for notational purposes, write \(q_i = q_0\) and \(q_f = q_N\) and similarly for \(T\). We then have for the amplitude 

\begin{align}
    A = \int dq_1 \ldots dq_{N-1}\prod_{m=0}^{N-1} \Braket{q_m T_m | q_{m+1} T_{m+1}}
\end{align}

How do we write this is the Schroedinger picture? Recall the "conversion relation" between the two pictures: 

\begin{align}
    \Braket{q_m T_m  | q_{m+1} T_{m+1}} = \bra{q_m} e^{-iH (T_{m+1} - T_m)}\ket{q_{m+1}}
\end{align}

Suppose \(T_i\) are all evenly spaced, say of \(\eps\), than we write 

\begin{align}
    \bra{q_m}e^{-iH\eps}\ket{q_{m+1}} 
\end{align}

We can also introduce an integration over the momentum states: 

\begin{align}
    \Braket{q_{m+1} T_{m+1} | q_{m} T_{m}} = \int dp_m \Braket{q_{m+1} | p_m} \bra{p_m}e^{-iH\eps}\ket{q_{m}}
\end{align}

This is convenient because \(H = H(p, q)\) and we can either act on the position ket or on the momentum bra, and in both cases we work with eigenstates. This is "legal" as we're summing over all momentum. 

If we normalize \(\Braket{q, p} = e^{ipq}\), then 

\begin{align}
    \Braket{q_{m+1} T_{m+1} | q_{m} T_{m}} = \int dp_m e^{iq_{m+1}p_m}\bra{p_m}e^{-iH(p_m, q_m)}\ket{q_m}
\end{align}

notice that now H is a function of the eigenvalue, not an operator anymore. 

Finally, we write all as a big exponential:  

\begin{align}
    \Braket{q_{m+1} T_{m+1} | q_{m} T_{m}} = \int dp_m \exp\left\{-i\eps\left(H(p_m, q_m) - \frac{q_{m+1}-q_m}{\eps}p_m\right)\right\}
\end{align}

If we do this for every therm in the amplitude A, we get 

\begin{align}
    A = \int dq_1 dp_1\ldots dq_{N-1}dp_{N-1} \exp\left\{-i\eps\sum_{m=0}^N\left(H(p_m, q_m) - \frac{q_{m+1}-q_m}{\eps}p_m\right)\right\}
\end{align}

Now perform the limit when \(\eps \rightarrow 0\): this means we're integrating over all possibile paths \(q(t)\), with fixed boundary conditions, i.e. \(q(T_i) = q_i, q(T_f) = q_f\). Defining \([dpdq] = \lim_{\eps \rightarrow 0}dq_1dp_1 \ldots dq_{N-1}dp_{N-1}\), we get: 

\begin{align}
    A = \int_{q(T_i) = q_i}^{q(T_f) = q_f} [dpdq] \exp\left\{i\int_{T_i}^{T_f} dt (p\dot{q} - H) \right\}
\end{align}

Normally, the integral over p is gaussian (from the kinetic part of H), and can be performed, leaving just a normalization that we put in \([dq]\). We also recognize the lagrangian in the integrand, so the integral is actually the classical action: 

\begin{align}
    A = \int [dq]\exp\left\{\frac{i}{\hbar}S \right\}
\end{align}

The classical limit is easily checked as \(\hbar \rightarrow 0\): if we consider a small change in the trajectory, \(q(t)\rightarrow q(t) +\delta q(t)\), this give large oscillations in the integrand, that average to zero. So the only contribution comes from the particular path that leaves the action unchanged under small oscillations, i.e. we recover the minimal action principle of classical mechanics.  

Consider now what happens if in the integrand of the path integral we insert a factor \(q(t)\), with \(t \in [T_i, T_f]\): 
...
\begin{align}
    \int [dq]= \int d\bar{q} \int_{q(T_i) = q_i}^{q(t) = \bar{q}} [dq] \int_{q(t) = \bar{q}}^{q(T_f) = q_f} [dq]
\end{align}

Split also the action integral to split the integrals: 

\begin{align}
    \int d\bar{q} \left(\int_{q(T_i) = q_i}^{q(t) = \bar{q}} [dq] e^{i\int_{T_i}^t L}\right)\bar{q}\left(\int_{q(t) = \bar{q}}^{q(T_f) = q_f} [dq] e^{i\int_{t}^{T_f} L}\right)
\end{align}

we recognize the terms in the parenthesis to be just amplitudes, 

\begin{align}
    \int d\bar{q} \Braket{q_i T_i | \bar{q} t} \bar{q}(t)\Braket{\bar{q} t | q_f T_f} = \bra{q_i T_i} q(t) \ket{q_f T_f}
\end{align}

where we used the completness relation. 

Importantly, the generalizations for this for an arbitrary number of functions is very useful for QFT. Consider 

\begin{align}
    \int [dq] q(t_1)\ldots q(t_N) e^{iS}
\end{align}

The N functions do not need to be put in a specific order, as they commute. However, when we repeat the steps above, we have to time order them to be able to split the integrals correctly. This means that 

\begin{align}
    \int [dq] q(t_1)\ldots q(t_N) e^{iS} = \bra{q_i T_i}T\left\{q(t_1)\ldots q(t_N) \right\}\ket{q_f T_f}
\end{align}

\section{From QM to QFT}

The generalization to QFT , at least to a formal level, is quite straightforward: one writes 

\begin{align}
    \Braket{\phi_f T_f | \phi_i T_i} = \int_{\phi(x, T_i) = \phi_i(x)}^{\phi(x, T_f) = \phi_f(x)} \mathcal{D}\phi e^{i \int d^4x \mathcal{L}} 
\end{align}

Of course, what one is really interested in QFT is using the LSZ formula to compute amplitudes, and in the LSZ formula appears the time ordered product 

\begin{align}
    \bra{\Omega}T\left\{\phi(x_1)\ldots \phi(x_n) \right\}\ket{\Omega}
\end{align}

now in the path integral, we can write 

\begin{align}
    \bra{\Omega}T\left\{\phi(x_1)\ldots \phi(x_n) \right\}\ket{\Omega} = \frac{\int \mathcal{D}\phi \phi(x_1)\ldots \phi(x_n)e^{iS}}{\int \mathcal{D}\phi e^{iS}}
\end{align}

We expect that if we make a perturbative expansion of the lagrangian in the exponential, we can recover the the Feynman rules as we did in the canonical quantisation. Moreover, we can also not expand the exponential, and treat the theory exactly, at least in principle. 

It is useful to introduce the \emph{generating functions}, that are useful tools to make this type of computations. To start, consider a scalar field theory and write

\begin{align}
    Z[J] = \int \mathcal{D}\phi \exp\left\{i(\mathcal{L} + \phi J )\right\}
\end{align}


Then, define the \emph{functional derivative} as the following: 

\begin{align}
    \frac{\delta}{\delta J(y)}J(x) = \delta(x-y)
\end{align}

The usual rules for derivatives hold, and we can also bring the derivative under the integral sign. For example, then, 

\begin{align}
    \frac{\delta}{\delta J(x_1)} \int d^4x \phi(x)J(x) = \phi(x_1)
\end{align}

So when we apply this to the generation functional, we see that 

\begin{align}
    \frac{\delta}{\delta J(x_1)}\int \mathcal{D}\phi e^{i\int d^4x (\mathcal{L} + J\phi)} = \int \mathcal{D}\phi (i\phi(x_1)) e^{i\int d^4x (\mathcal{L} + J\phi)}
\end{align}

From this simple rule, we derive 

\begin{align}
    \left(\frac{-i\delta}{\delta J(x_1)} \right)\ldots\left(\frac{-i\delta}{\delta J(x_N)} \right)Z[J]|_{J=0} = \int \mathcal{D}\phi \phi(x_1)\ldots \phi(x_N)e^{iS}
\end{align}

\ex{The scalar free theory case, Feynman propagator from path integral}{

    Consider a free real scalar theory, with lagrangian 

    \begin{align}
        \mathcal{L} = \frac{1}{2}(\partial_\mu\phi \partial^\mu\phi -(m^2-i\eps)\phi^2)
    \end{align}

    we inserted a little imaginary shift in the mass, to be sure that the integrals converge if \(\phi\) gets big. 

    To begin, we need to compute the generator: 

    \begin{align}
        Z[J] &= \int \mathcal{D}\phi e^{i\int d^4x (\mathcal{L} + \phi J)} \\
        &= \int \mathcal{D}\phi e^{(i/2)\int d^4x (\partial_\mu\phi \partial^\mu\phi -(m^2+i\eps)\phi^2 + 2\phi J)}
    \end{align}

    If we integrate by parts, we find the explicit form of the KG operator: 

    \begin{align}
        Z[J] = \int \mathcal{D}\phi e^{(i/2)\int d^4x (\phi(-\square -m^2+i\eps)\phi + 2\phi J)}
    \end{align}

    Now shift the fields \(\phi \rightarrow \phi' = \phi + i \int d^4y D(x-y)J(y)\), where \(D(x-y)\) is the green function of the KG equation. 

    Look closely at the first term in the exponential: it transforms as 
    
    \begin{align}
        \phi'(-\square - m^2)\phi' + i \int d^4y \phi' (-\square -m^2)D(x-y)J(y) + i \int d^4y D(x-y)J(y)(-\square -m^2)\phi'  \\ - \int d^4yd^4y' D(x-y)J(y)(-\square-m^2)D(x-y')J(y')
    \end{align}

    Now, we act with the KG operator on the green function. In the third term to do this we IBP two times, and drop the boundary terms. In the end, the second and third term exactly cancel out the \(2\phi J\) factor, so we're left with 


    \begin{align}
        Z[J] = \int \mathcal{D}\phi e^{id^4x \mathcal{L}}e^{(-1/2)\int d^4x d^4y J(x)D(x-y)J(y)} = Z[J=0]e^{i\int d^4x d^4y J(x)D(x-y)J(y)} 
    \end{align}

    Now the generating functional is in a much nicer form, and we can apply the derivatives. Recall that 

    \begin{align}
        \bra{\Omega}T\left\{\phi(x_1)\phi(x_2) \right\}\ket{\Omega} =  -\frac{\delta}{\delta J(x_1)}\frac{\delta}{\delta J(x_2)}\frac{Z[J]}{Z[J=0]}
    \end{align}

    Now it's time to apply the derivatives. From the first one, 

    \begin{align}
        \frac{\delta}{\delta J(x_2)}\exp\left\{-\frac{1}{2}\int d^4xd^4y J(x)D(x-y)J(y) \right\} = \\ =  -\frac{1}{2}\left(\int d^4y D(x_2-y)J(y) + \int d^4x J(x)D(x-x_2) \right)\exp\left\{-\frac{1}{2}\int d^4xd^4y J(x)D(x-y)J(y) \right\}
    \end{align}

    Now we apply the second one. If we act again on the exponential, that's going to give as terms proportional to \(J\), that we will put to zero in the step after. So we only really need to worry about the terms in the parenthesis, that can be also rewritten as 

    \begin{align}
       \frac{\delta}{\delta J(x_1)} \left(-\int d^4y D(x_2-y)J(y)\right) = -D(x_1-x_2)
    \end{align}

    So in the end, 

    \begin{align}
        \bra{\Omega}T\left\{\phi(x_1)\phi(x_2) \right\}\ket{\Omega} = D(x_1-x_2)
    \end{align}
...
\section{Functional quantisation of fermion fields}

We saw also in the canonical quantisation that fermions are different from bosons. There, to quantize them, we used anticommutation relations, and not commutation relations as we did for example for scalar theories, or the electromagnetic field. 

For this reason, we have to introduce a set of \emph{anti-commuting} numbers. 


\subsection{Grassman numbers}

Consider a set of numbers \(\left\{\psi_i \right\}, \, i \in 1, \ldots, N\), that obey 

\begin{align}
    \left\{\psi_i, \psi_j \right\} = 0 
\end{align}

For example, if \(N=1\), we simply have \(\psi^2 = 0\).
Things get interesting when we consider an arbitrary smooth function of \(\psi\). As \(\psi^2 = 0\), the Taylor expansion of the function is exact in every point, and it's just \(f(\psi) = a+b\psi\). \\

We'd like to impose that \(f\) commutes with itself. What does this impose for \(b\)? Is it a commuting or anticommuting number? To find out, write \(bb' = \lambda bb'\), as well as \(\psi b = \lambda b \psi\), with \(\lambda\) either \(1\) or \(-1\). Then, 

\begin{align}
    (\psi' b')(\psi b) = \lambda^2 \psi' \psi b b' = -\lambda^2 \psi \psi' b b' = -\lambda^3 \psi b \psi' b' 
\end{align}

so in order to get \(\psi' b' \psi b = \psi b \psi ' b'\), \(\lambda\) must be \(-1\). So b itself is also a Grassman number. 

We now want to define calculus on these numbers. Let's start with derivatives. There are two types of derivatives: 

\begin{align}
    \partial_\psi (a+ \psi b) = b 
\end{align}


on the other end, we can also define 

\begin{align}
    (a+ \psi b)\partial^\leftarrow_\psi = (a-  b \psi)\partial^\leftarrow_\psi = -b
\end{align}

Now, let's define integration. This means we'd want to give a meaning to the symbol 

\begin{align}
    \int_{-\infty}^{+\infty} d\psi f(\psi)
\end{align}

Let's start with some basic assumptions that every type of integral should follow: we impose that's linear, and invariant under translations. In formulas, we impose: 

\begin{align}
    \begin{cases}
        \int d\psi cf(\psi) = c\int d\psi f(\psi) \\
        \int d\psi f(\psi +\eta) = \int d\psi f(\psi)
    \end{cases}
\end{align}

Of course, the integral should also be a function of both \(a, b\). Under a translation, though, we have 

\begin{align}
    f(\psi) = a +b\psi \rightarrow f(\psi +\eta) = a + \eta b + b\psi = a' + b\psi
\end{align}

with \(a' = (a+b\eta)\). So the integral can be a function of b alone. Choosing the normalization to be \(1\), we simply define 

\begin{align}
    \int d\psi f(\psi) = b
\end{align}

What happens if we have more than one Grassman variable? Then the Taylor expansion can involve the product of different Grassman numbers, as 

\begin{align}
    f(\psi) = a + \psi_i b_i + \frac{1}{2}\psi_{i_1}\psi_{i_2}C_{i_1, i_2} + \ldots + \frac{1}{n!}D_{i_1, \ldots i_n}\psi_{i_1}\ldots\psi_{i_n}
\end{align}

but it's still exact and finite, and of order \(N\). Differentiation now works as 

\begin{align}
    \frac{\partial}{\partial \psi_j}f(\psi) = b_j + \psi_i C_{ij} + \ldots + \frac{1}{(n-1)!}\psi_{i_1}\ldots\psi_{i_{j-1}}\psi_{i_{j+1}}\ldots \psi_{i_n}D_{i_1, \ldots, i_n}
\end{align}

The only integral that follows our rules is given by 

\begin{align}
    \int d^n \psi f(\psi) = d, \quad \text{as} \quad d_{i_1, \ldots, d_n} = d\eps_{i_1, \ldots i_n}
\end{align}

Before going back to fermions, we list a series of known results for integrals that will be useful when we'll have to compute the actual path integrals. 

First consider a linear transformation 

\begin{align}
    \psi_i = J_{ij}\psi'_j 
\end{align}

where \(J\) is a matrix made up of commuting numbers. Then, 

\begin{align}
    \int d\psi' f(\psi') = \det(J)\int d^n \psi f(\psi)
\end{align}

notice that is the other way around respect to standard numbers. Similarly, the gaussian integral is given by

\begin{align}
    \int d^n \psi \exp\left[\frac{1}{2}\psi^T M \psi\right] = \sqrt{\det(M)}
\end{align}

Finally, we can pair two Grassman numbers to get a Grassman complex number, simply as 

\begin{align}
    \chi = \frac{1}{\sqrt{2}}(\psi_1 + i\psi_2), \quad \bar{\chi} = \frac{1}{\sqrt{2}}(\psi_1 - i\psi_2)
\end{align}

And in this case, we get 

\begin{align}
    \int d^n \chi d^n \bar{\chi} e^{\chi^\dagger M \chi} = \det M \\
    \int d^n \chi d^n \bar{\chi} e^{\chi^\dagger M \chi + \eta^\dagger \chi + \chi^\dagger \eta} = \det M e^{-\eta^\dagger M^{-1}\eta} \\
    \int d^n d^n \bar{\chi} \chi \bar{\chi} e^{\chi^\dagger M \chi} = \det M (M)^{-1}
\end{align}

\subsection{The fermion propagator}

In the path integral formalism, if we want to compute the feynman propagator for a Dirac field, with free lagrangian given by 
...
\begin{align}
    \bra{0}T\left\{\psi(x_1)\bar{\psi}(x_2) \right\}\ket{0} = \frac{\int \mathcal{D}\bar{\psi}\mathcal{D}\psi \bar{\psi}\psi e^{i\int d^4x \bar{\psi}(i\slashed{\partial} - m)\psi}}{\int \mathcal{D}\bar{\psi}\mathcal{D}\psi e^{i\int d^4x \bar{\psi}(i\slashed{\partial} - m)\psi}}
\end{align}

in the lhs, the Dirac fields are written as 

\begin{align}
    \psi = \sum_i \psi_i \phi(x)
\end{align}

so that if we invert the time ordering, both sides of the equation pick up a minus sign. (Remember that \(T\left\{\psi_1 \bar{\psi}_2 \right\} = -\bar{\psi}_2 \psi_1\), if \(x_2^0 > x_1^0\)). 

Then the equation is well defined, and we compute it using the formulas that we listed previously: 

\begin{align}
    \bra{0}T\left\{\psi(x_1)\bar{\psi}(x_2) \right\}\ket{0} = \frac{\det (i\slashed{\partial}-m) (i\slashed{\partial}-m)^{-1}}{\det (i\slashed{\partial}-m)}  = (i\slashed{\partial}-m)^{-1}
\end{align}

Now recall that \(S_f(x_1-x_2)\) is the green function of Dirac's equation, meaning that 

\begin{align}
    (i\slashed{\partial}-m)S_f(x_1-x_2) = i\delta(x_1-x_2) \implies (i\slashed{\partial}-m)^-1 = S_f(x_1-x_2) \implies  \bra{0}T\left\{\psi(x_1)\bar{\psi}(x_2) \right\}\ket{0} = S_f(x_1-x_2)
\end{align}

as we expected. 

\chapter{UV divergences}

\section{1 Loop corrections in \(\phi^4\) theory}

Let's go back to the canonical quantisation, and recall that we evaluated the amplitude of the process \(\phi\phi \rightarrow \phi\phi\) up to order 1 in \(\lambda\). What happens if we want to compute higher order corrections? The LSZ formula then requires us to compute 

\begin{align}
    \frac{(i\lambda)^2}{2(4!)} \bra{\Omega}T\left\{\phi(x_1)\phi(x_2)\phi(y_1)\phi(y_2) \int d^4z \phi^4(z) \int d^4z' \phi^4(z')\right\}\ket{\Omega}
\end{align}

Performing the computation with Wick's theorem is possibile, but not very practical. Instead, we can think at this with Feyman diagrams: then we find that the order 2 corrections all contain a loop in the diagram. In those loops, the momentum is underdetermined, meaning that the conservation of the momentum at every vertex + the conservation of external momenta is not sufficient to completely determine the momenta inside the loops. For this reason, we must integrate over all undetermined momentum. This is the new Feynman rule that appears when we consider diagrams that contain loops. 

As all Feynman rules, this comes directly from the LSZ formula. To see this, consider for example the possibile contraction 

\begin{align}
    -\frac{\lambda}{2 (4!)}\int d^4z d^4z' D(x_1-z)D(x_2-z)D(z-z')D(z-z')D(y_1-z')D(y_2-z')
\end{align}

As we always do, we translate the variables and perform the first integrals, to get 

\begin{align}
    \tilde{D}(p_1)\tilde{D}(p_2)\tilde{D}(k_1)\tilde{D}(k_2)\int d^4z d^4z' e^{iz(p_1+p_2)}e^{-iz'(k_1+k_2)}D(z-z')D(z-z')
\end{align}

Switch to the fourier transform in the integrals: 

\begin{align}
    \tilde{D}(p_1)\tilde{D}(p_2)\tilde{D}(k_1)\tilde{D}(k_2)\int d^4z d^4z' \frac{d^4\ell_1}{(2\pi)^4}\frac{d^4\ell_2}{(2\pi)^4}e^{iz(p_1+p_2)}e^{-iz'(k_1+k_2)}\tilde{D}(\ell_1)D(\ell_2)e^{i\ell_1(z-z')}e^{i\ell_2(z-z')}
\end{align}

Now reorder the arguments in the exponentials, to get two Dirac deltas: 

\begin{align}
    \tilde{D}(p_1)\tilde{D}(p_2)\tilde{D}(k_1)\tilde{D}(k_2)\int \frac{d^4\ell_1}{(2\pi)^4}\frac{d^4\ell_2}{(2\pi)^4}\tilde{D}(\ell_1)\tilde{D}(\ell_2)(2\pi)^8\delta(p_1+p_2-\ell_1-\ell_2)\delta(k_1+k_2+\ell_1+\ell_2)
\end{align}

Finally, use the first one to get 

\begin{align}
    (2\pi)^4\delta(p_1+p_2-k_1-k_2)\tilde{D}(p_1)\tilde{D}(p_2)\tilde{D}(k_1)\tilde{D}(k_2)\int \frac{d^4\ell}{(2\pi)^4}\tilde{D}(\ell)\tilde{D}(p_1+p_2-\ell)
\end{align}

So we verified that we indeed have to integrate over the underdetermined momentum in this case. 

Now use the rules to write the amplitude, relative to the same diagram:

\begin{align}
    A(p_1+p_2) \propto \frac{i\lambda}{2}\int \frac{d^4\ell}{(2\pi)^4}\frac{i}{(p_1+p_2-\ell)^2-m^2}\frac{i}{\ell^2-m^2}
\end{align}

This is divergent in the UV limit. (that is, when momentum goes to infinity). This divergence though is not trivial. This is because \(\ell\) is a four vector living in Minkoswki spacetime. To proceed, then, first consider \(A(0)\): 

\begin{align}
    A(0) = \frac{(i\lambda)^2}{2}\int \frac{d^4\ell}{(2\pi)^4}\left(\frac{i}{\ell^2-m^2+i\eps} \right)^2 = \frac{\lambda^2}{2}\int \frac{d^3\ell}{(2\pi)^3}\int \frac{d\ell_0}{(2\pi)}\frac{1}{(\ell_0^2 - (\vec{\ell})^2 -m^2 + i\eps)^2}
\end{align}

consider computing the last integral with the residue theorem. Remember that the Feynman prescription displaces slightly the two poles from the real axis, the positive pole below, the negative one above. So it is clear that the last integral is the same as another integral, the one of the same function, on the \emph{imaginary} axis, from \(-i\infty\) to \(i\infty\)

Thus, the residue theorem tells us that we can safely write 

\begin{align}
    A(0) = \frac{\lambda^2}{2}\int \frac{d^3\ell}{(2\pi)^3}\int_{-i\infty}^{i\infty} \frac{d\ell_0}{(2\pi)}\frac{1}{(\ell_0^2 - (\vec{\ell})^2 -m^2 + i\eps)^2}
\end{align}

It is now useful to change the integration variable. Use \(\ell_E^0 = -i\ell_0\). Then we get (writing \(\vec{\ell}_E = \vec{\ell}\)): 

\begin{align}
    A(0) = \frac{-i\lambda^2}{2}\int \frac{d^4\ell_E}{(2\pi)^4}\frac{1}{(\ell_E^2 +m^2)^2} = \frac{-\lambda^2}{2}\int_0^\Lambda \frac{d\Omega_3 d\ell_E}{(2\pi)^4}\frac{\ell_E^3}{(\ell_E^2+m^2)^2} \propto \log(\Lambda)
\end{align}

where we insterted a cut-off scale \(\Lambda\). Thus, we say that the diagram diverges logarithmically. 

This is not the only type of one-loop correction that appears in the theory. There are also the corrections to the propagatore, the so called "tadpole" diagrams, that have amplitude 
...
\begin{align}
    -iB = \frac{-i\lambda}{2} \int \frac{d^4\ell}{(2\pi)^4}\frac{i}{\ell^2-m^2+i\eps} = \frac{-i\lambda}{2} \int \frac{d^4\ell_E}{(2\pi)^4}\frac{1}{\ell_E^2+m^2} = \frac{-i\lambda}{4(2\pi)^2}\int d\ell_E \frac{\ell_E^3}{\ell_E^2+m^2}
\end{align}

change integration variable from \(\ell_E\) to \(\ell_E^2+m^2\), and introducing again a cut-off scale \(\Lambda\) we find 

\begin{align}
    -iB = \frac{-i\lambda}{2(2\pi)^2}\int_{m^2}^{\lambda^2+m^2} dx \frac{x-m^2}{x} = \frac{-i\lambda}{2(2\pi)^2}\left[\Lambda^2 - m^2\log\left(\frac{\Lambda^2+m^2}{m^2} \right) \right]
\end{align}

Actually, it's possible to compute all tadpole contributions, at all orders in \(\lambda\), to the two point function. Consider 

\begin{align}
    D(p) + D(p)(-iB)D(p) + D(p)(-iB)D(p)(-iB)D(p) + \ldots &= D(p)(1 + (-iB)D(p) + ((-iB)D(p))^2 + \ldots)  \\ &= \frac{D(p)}{1+iBD(p)} = \frac{i}{p^2-m^2}\frac{1}{1 - \frac{B}{p^2-m^2}} = \frac{1}{p^2 - m^2 - B}
\end{align}

so we see that the effect of introducing this contributions is shiting the mass in the propagator from \(m^2 \rightarrow m^2 + B\), with \(B\) being infinite! We'll see how we deal with this when we will talk about renormalization. 

\section{Counting the infinites}

Is there a way of determining with what degree is a diagram going to diverge, without having to write the full amplitude of a feynman diagram? the answer is yes, and this trick is useful also for being able to predict if at a given order you're going to have UV divergences or not. 

Consider again \(\phi^n\) theory. Since the vertex rules don't involve momentum, the degree of divergence of a diagram is given by the numer of: 

\begin{enumerate}
    \item Loops: with every loop comes an integration over the \(d\)-dimensional spacetime. 
    \item Propagators: each propagator has \(p^{-2}\) dependence, at least in the UV limit where masses don't count. 
\end{enumerate}

So we can define the degree of divergence to be \(D = dL-2P\). Can we say something else? 

Well, in three level diagrams, you always have \(V = P+1\), if \(V\) is the number of vertices. From a tree level diagram, you have two options: 

\begin{enumerate}
    \item add another vertex and propagator, and this gives again a tree level diagram 
    \item add two vertexes and one internal line (propagator)
\end{enumerate}

From this two choices, we get the rule for the number of loops in the graph: 

\begin{align}
    L = P- V+1
\end{align}

now, every vertex is joined by n-lines in this theory. External lines are linked only ones, internal (the propagators) two, so we get 

\begin{align}
    nV = N_e +2P \implies P = \frac{1}{2}(nV - N_e)
\end{align}

We can then write the degree of divergence to be: 

\begin{align}
    D &= d(P-V+1) - 2P = P(d-2) -dV +d \\
    &= \frac{(d-2)}{2}(nV-N_e) -dV + d \\
    &= V\left(n\frac{d-2}{2} -d \right) -\frac{d-2}{2}N_e + d
\end{align}

Notice that in perturbation theory, you get higher in order by adding vertexes. So the sign of the coefficient 

\begin{align}
    \left(n\frac{d-2}{2} -d \right)
\end{align}

is very important. If it's positive, the orders of divergence gets bigger in all orders, and those theories are called "non-renormalizable". Instead, if the coefficient is zero, than the number of diverge diagrams is finite at avery order in perturbation theory, and the theory is called renormalizable. Instead, if it's negative, than after some order, there are not divergences anymore. Those are called super-renormalizable theories. 

Notice that the coefficient is exactly minus the mass dimension of the coupling costant \(\lambda\). This is true for every theory, the mass dimension of the theory already gives information on the number of divergences that will appear in high orders of perturbation theory.  

\section{Renormalization}

Consider again the problem that we encountered considering tad-pole diagrams: the re-summed propagator was given by 

\begin{align}
    \frac{i}{p^2-m_0^2-B +i\eps}
\end{align}

if we write the lagrangian as 

\begin{align}
    \frac{1}{2}(\partial_\mu\phi_0 \partial^\mu \phi_0 - m_0^2\phi_0^2) + \frac{\lambda_0}{4!}\phi_0^4 
\end{align}

now, \(m_0^2\) is just a parameter in the lagrangian (sometimes called the "bare" mass). What really interests us, is the pole of the propagator, because that enters in amplitudes and can be measured. So we can choose the parameter to be such that \(m_0\) cancels the divergence of \(B\) and gives the physical mass \(m = \sqrt{m^2+B}\) that we actually measure.  

The situation gets slightly more difficult if we consider two loops correction to the propagator. This time, the re-summed propagator becomes 

\begin{align}
    \frac{i}{A(p^2, m_0^2)p^2-m_0^2-B+i\eps}
\end{align}

We still want the propagator to have a pole in \(p^2 =m^2\). So we require that

\begin{align}
    (A(p^2, m_0^2)p^2-m_0^2-B)|_{p^2 =m^2} = 0 
\end{align}

now, i want to see how the propagator looks like near the pole, so I expand: 

\begin{align}
    \frac{i}{A(p^2, m_0^2)p^2-m_0^2-B} \simeq \frac{i}{(\frac{d}{dp^2}A(p^2, m_0^2)p^2)(p^2-m^2)} + \ldots = \frac{iZ}{p^2-m^2} + \ldots
\end{align}

where \(Z\) is called the wave function renormalization constant. This is because if we define the normalized fields as 

\begin{align}
    \phi = Z^{1/2}\phi_0 
\end{align}

then this normalization factors cancel out in the LSZ formula.



By scaling the mass and renormalizing the fields, we have thus cured the divergence of the 2 point function. What about the other divergence, the logartmic one that appeared when we had to compute the second order corrections for \(\phi\phi \rightarrow \phi\phi\)? Up to second order, we had 

\begin{align}
    i\mathcal{M}(p_1p_2 \rightarrow k_1k_2) = -i\lambda_0 + (i\lambda_0)^2(\beta \log(\Lambda) + \ldots)
\end{align}

We'd like to impose 

\begin{align}
    i\mathcal{M}(p_1p_2 \rightarrow k_1k_2) = i\lambda
\end{align}

but we must choose a particular configuration, for example 

\begin{align}
    p_1 = p_2 = k_1 = k_2 = (m, \vec{0})
\end{align}

now the only scales are \(\Lambda\) and \(m\), so 

\begin{align}
    -i\lambda = -i\lambda_0(1-\lambda_0(\Lambda)(\beta\log(\Lambda^2/m^2)) + o(\lambda^3))
\end{align}

Even if this is fixed only for a particular configuration, the matrix element is finite for every sale. Consider for example if \((p_1+p_2)^2 >> m^2 \), then that is the only scale and 

\begin{align}
    iM(q^2) &= -i\lambda_0(1-\lambda_0(\beta\log(\Lambda^2/q^2))) \\
    &= -i\lambda_0(1-\lambda_0(\beta(\log(\Lambda^2/m^2) - \log(m^2/q^2))) \\
    &= -i\lambda - i\lambda_0(1-i\lambda_0\beta\log(m^2/q^2))
\end{align}

so we say that after renormalization, the coupling constants become running. 
