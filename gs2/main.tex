...
\author{Giacomo Galliani, corso del prof. A. Amariti,  A. Santambriogio}
...
%\part{Introduzione alla teoria delle stringhe}
...
%\part{Introduzione alla Supersimmetria}

\chapter{Le superalgebre di Lie}

\section{Motivazione allo studio della supersimmetria e teorema di Coleman-Mandula}

Perché i fisici nello scorso secolo si sono interessati così tanto alla supersimmetria? Vi sono molte ragioni teoriche, anche di carattere molto pratico. Cominciamo con una di carattere più filosofico: il modello standard delle particelle elementari (SM) ha due simmetrie: una di spaziotempo, cioè la simmetria sotto il gruppo di Poincaré, ed un altra invece detta interna, cioè la simmetria di gauge sotto il gruppo \(SU(3)\times SU(2)\times U(1)\). Possiamo indicare l'algebra globale come \(\mathcal{P} + g\). Vorremmo unificare queste simmetrie, all'interno di una algebra \(\mathcal{A}\) più grande, che le contiene (\(\mathcal{P} \oplus g \subset A\)), in modo non banale.  

Inoltre, lo SM ha due tipi di particelle: fermioni e bosoni. Se si trovasse una simmetria che connettesse questi due tipi di particelle così diverse, sarebbe un grande progresso teorico. Un ultima motivazione sono le divergenze quadratiche del SM. Ad esempio, se uno calcola i controtermini alla massa di Higgs, \(\delta M_H\), diciamo introducendo come tecnica di regolazione un cut-off, allora si trova che \(\delta M_H \propto \Lambda^2\). In teoria dei campi si dice che la teoria ha delle divergenze quadratiche. Per lo SM, \(\Lambda \approx 10^{14/15}\) GeV. Si ha quindi che la sottrazione di due termini enormi, dia luogo ad una massa \(M_H \approx 100\) GeV. Questo non è impossibile, ma poco probabile: si tratta del cosiddetto \emph{problema della gerarchia}. Il problema non si pone in SUSY perché le divergenze sono al massimo logaritmiche. Infine, nello SM si pensa che le tre costanti \(e, g_W, g_s\) tendano, con l'aumento dell'energia, allo stesso valore \(g'\). Sembra che la convergenza sia più semplice se si assume che la teoria è supersimmetrica. 

Come possiamo allargare la simmetria di una teoria, nel modo non banale descritto all'inizio? Cominciamo a studiare una teoria di uno scalare autointeragente, con la seguente azione: 

\begin{align}
    S = \int d^4 x (\partial_\mu \phi)(\partial^\mu \phi)^\dagger - m^2 \phi \phi^\dagger - \lambda^2(\phi\phi^\dagger)^2
\end{align}

Quali sono le simmetrie della teoria? Certamente abbiamo la simmetria di Poincaré, \(\mathcal{P}\). Inoltre, considera la trasformazione \(\phi \to e^{i\alpha}\phi\). Si tratta di una trasformazione globale di fase, cioè del gruppo \(U(1)\), ed è immediato vedere che realizza una simmetria della lagrangiana. 

Queste simmetrie portano (teo. di Noether) delle cariche conservate. Osserviamole: 

\begin{enumerate}
    \item Simmetrie di spaziotempo \(\mathcal{P}\): Abbiamo invarianza sotto traslazioni spaziotemporali, che porta il tensore di energia momento ad avere divergenza nulla, \(\partial_\mu T^{\mu\nu} = 0\). Definendo poi \(P^\nu = \int d^3x T^{0\nu}\), si ottiene che \(\frac{d}{dt}P^\nu = 0\). Invece, la simmetria rispetto alle trasformazioni di Loretnz porta alla conservazione del momento angolare: vale \(\partial_\mu J^{\mu, \nu \rho} = 0\), definendo \(J^{\nu\rho} = \int d^3x J^{0, \nu\rho}\), si ottiene allora \(\frac{d}{dt}J^{\nu\rho} = 0\).
    \item Dalla simmetria interna, la corrente conservata è data da \(j^\mu = i(\phi(\partial^\mu\phi^\dagger) - (\partial^\mu\phi)\phi^\dagger)\). Infatti, si calcola immediatamente la sua divergenza, 
    \begin{align}
        \partial_\mu j^\mu = i(\phi \square \phi^\dagger - (\square \phi)\phi^\dagger)
    \end{align}
    Per sapere come è fatto \(\square \phi\), calcola le equazioni del moto: 
    \begin{align}
        \frac{\partial \mathcal{L}}{\partial \phi} = \partial_\mu \frac{\partial \mathcal{L}}{\partial (\partial_\mu\phi)} \implies -m^2\phi^\dagger -2\lambda\phi(\phi\phi^\dagger) = \square \phi^\dagger \implies \begin{cases}
            \phi\square\phi^\dagger = -m^2\phi\phi^\dagger - 2\lambda(\phi\phi)^\dagger \\
            \phi^\dagger\square\phi = -m^2\phi\phi^\dagger - 2\lambda(\phi\phi)^\dagger
        \end{cases}
    \end{align}
    Immediatamente vediamo che la corrente è conservata. Come solito definiamo allora \(Q = \int d^3x j^0\), così che \(\frac{d}{dt}Q = 0\). 
\end{enumerate}

Come è fatta quindi l'algebra di simmetria? L'algebra di Poincaré è la solita, 

\begin{align}
    &[P_\mu, P_\nu] = 0 \notag \\
    &[P_\mu, J_{\nu\rho}] = \eta_{\mu\nu}P_\rho - \eta_{\mu\rho}P_\nu \notag \\ 
    &[J_{\mu\nu}, J_{\rho\sigma}] = \eta_{\mu\nu}J_{\rho\sigma} +\eta_{\nu\rho}J_{\mu\sigma} - \eta_{\mu\rho}J_{\nu\sigma} -\eta_{\nu\sigma}J_{\mu\rho}
\end{align}

Invece la carica \(Q\) realizza l'algebra di \(\mathfrak{u}\)(1), poiché semplicemente \([Q, Q] =0\). Inoltre, poiché vale 
...
L'algebra totale è la somma diretta, \(\mathcal{P} \oplus \mathfrak{u}\)(1). Vorremmo allargare questa algebra di simmetria. Come facciamo? Osserviamo che è molto semplice allargare la simemtria interna, ad esempio ampliando \(\mathfrak{u}(1) \to \mathfrak{u}(N)\). Basta scrivere la lagrangiana in termini di un multipletto di \(N\) campi: 

\begin{align}
    S = \int d^4x (\partial_\mu \phi) \partial^\mu \phi^\dagger -m^2 \phi \phi^\dagger - \lambda (\phi\phi^\dagger), \quad \phi = \begin{pmatrix}
        \phi_1 \\ \ldots \\ \ldots \\ \phi_N
    \end{pmatrix} \in \mathbb{C}^N, \quad \phi^\dagger = (\phi_1^*, \ldots, \phi_N^*)
\end{align}

La azione è allora simmetrica sotto la trasformazione dei campi \(\phi \to U\phi, \, \phi^\dagger \to \phi^\dagger U^\dagger\), a patto che valga \(UU^\dagger =1\), cioè \(U \in U(N)\). Quali sono adesso le cariche conservate? Se si definisce 

\begin{align}
    Q_a = i\int d^3x \phi^\dagger T_a \overleftrightarrow{\partial}_0 \phi
\end{align}

Se \(T_a\) è una base hermitiana dell'algebra (dunque una matrice \(N\times N\)). Si hanno quindi \(N^2\) cariche conservate. Le cariche soddisfano l'albegra di \(\mathfrak{u}\)(N), cioè vale \([Q_a, Q_b] = f_{abc}Q_c\). Ancora una volta come prima, vale 
...
Cioè l'algebra globale è sempre \(\mathcal{P}\oplus \mathfrak{u}(N)\). Se allargare il gruppo di simmetria interno è apparentemente semplice, vediamo che è più complicato introdurre una algebra di simmetria che sia diversa da un gruppo interno in somma diretta con Poincaré. Immaginare una teoria dove la carica non commuti con le traslazione è abbastanza difficile. Invece potremmo pensare ad na teoria dove vale \([Q_a, J^{\mu\nu} \neq 0]\). Questo vuol dire che la carica non è più uno scalare di spaziotempo. D'altra parte, questo vuol dire avere una carica vettoriale, o tensoriale, oltre a \(P^\mu, J^{\mu\nu}\). Questa simmetria dovrebbe anche collegare stati di spin diverso, perchè 

\begin{align}
    Q_\mu \ket{0} = \ket{1}
\end{align}

Se \(\ket{0}\) ha spin 0 e \(\ket{1}\) ha spin 1. 

Coleman e Mandula, nel 1967, hanno dimostrato che, con eccezione di casi particolari e poco importanti, la risposta a questa domanda è no. 

La dimostrazione del teorema è complicata. Facciamo comunque qualche commento: inanzitutto l'idea è che se esistessero cariche conservate di tipo tensoriale o vettoriale, queste vincolerebbero troppo la dinamica, e per forza di cose si andrebbe a cadere nella teoria libera, non interagente. Per vederlo facciamo un esempio: 

\ex{Conseguenze alla dinamica di una carica conservata tensoriale}{

    Supponiamo che in una data teoria \(Q_{\mu\nu}\), un tensore simmetrico e a traccia nulla, sia conservato. Considera anche uno stato ad una particella, con momento \(p^\mu\), che indichiamo con \(\ket{p}\). Considera il suo valore di aspettazione: deve avere una forma quadratica nei momenti, ed essere senza traccia, perciò varrà 

    \begin{align}
        \bra{p}Q_{\mu\nu}\ket{p} \propto (p_\mu p_\nu - \frac{1}{4}\eta_{\mu\nu}p^2)
    \end{align}

    Chiaramente \(p^2 = m^2\) è una quantità conservata. Segue quindi che si deve conservare anche \(p_\mu p_\nu\). Vediamo che conseguenze ha questo ad esempio in uno scattering \(2\to 2\). Se indichiamo con \(P_i\) i momenti entranti e \(P'_i\) i momenti esterni, questi devono soddisfare sia la conservazione del momento, sia la nuova legge: 
    ...
    \begin{align}
        \begin{cases}
            ...
            p_1^\mu p_1^\nu + p_2^\mu p_2^\nu = p_1^{\mu'} p_1^{\nu'} + p_2^{\mu'} p_2^{\nu'}
        \end{cases} \implies \begin{cases}
            p_1^\mu = p_1^{\mu'} \\ p_2^\mu = p_2^{\mu'} 
        \end{cases}, \quad \text{oppure} \quad \begin{cases}
            p_1^\mu = -p_1^{\mu'} \\ p_2^\mu = -p_2^{\mu'} 
        \end{cases}
    \end{align}

    Chiaramente, entrambi i casi sono troppo banali. Per questo motivo, il teorema di Coleman Mandula richiede nelle ipotesi che le soluzioni della teoria siano smooth (cioè lisce) nei parametri come gli angoli di scattering. 
}

Quali sono le eccezioni del teorema di Coleman Mandula? Ne citiamo alcune: 

\begin{enumerate}
    \item La teoria \emph{libera}: in questo caso, è semplice costruire teorie con cariche vettoriali o tensoriali conservate. Consideriamo ad esempio la seguente azione: 
    \begin{align}
        S = S_0 + S_1, \quad S_0 = \int d^4x \frac{1}{2}[\partial_\mu\phi\partial^\mu\phi -m^2\phi^2], \, \phi \in \mathbb{R}, \quad S_1 = \int d^4x \left[-\frac{1}{4}F_{\mu\nu}F^{\mu\nu} + \frac{1}{2}m^2 A_\mu A^\nu \right]
    \end{align}

    Dove, come al solito, \(F_{\mu\nu} = \partial_\mu A_\nu - \partial_\nu A_\mu\). La teoria è una teoria libera di campo scalare + una teoria libera di un campo vettoriale massivo. Le equazioni del moto che seguono dall'azione sono
    \begin{align}
        (\square +m^2)\phi = 0, \quad (\square +m^2)A_\mu = 0 
    \end{align}

    Da queste è immediato verificare che la corrente \(j_{\mu\nu} = \phi \overleftrightarrow{\partial}_\mu A_\nu\) è conservata. Segue che esiste una carica vettoriale conservata, data da 

    \begin{align}
        Q_\nu = \int d^3x j_{o\nu} \implies \frac{d}{dt}Q_\nu = 0 
    \end{align}

    In effetti è facile mostrare che esistono \emph{infinite} correnti conservate, date da \(\phi \overleftrightarrow{\partial}_{\mu_1} \ldots \overleftrightarrow{\partial}_{\mu_N}A_\nu\). Nel caso della teoria interagente però le e.o.m. diventano più complicate, e il trucco non funziona più. 
    \item Una teoria in sole \(d = 2\) dimensioni (una temporale e una spaziale). Il ragionamento per il quale le soluzioni devono essere smooth nei parametri non funziona più se la dimensione spaziale è solo una. Un esempio importante è il World-sheet in teoria delle stringhe: lì ci sono infinite cariche vettoriali/tensoriali conservate. 
    \item La teoria non relativistica: ad esempio nel Weimberg si mostra come valga 
    \begin{align}
        SU(3) \otimes SU(2) \subset SU(6)
    \end{align}
    Dove \(SU(3)\) è il gruppo interno di simmetria di sapore di Gell-Mann, mentre \(SU(2)\) è il gruppo di spin, cioè la simmetria spaziotemporale non relativistica. 
\end{enumerate}

Un'altra osservazione riguarda le teorie \emph{conformi}: queste hanno una simmetria interna più grande di quella di Poincaré, cioè una invarianza di scala. Queste hanno una carica conservata vettoriale in più, solitamente denotata con \(k^\mu\). Anche in questo caso, vale il teorema di Coleman-Mandula, a patto di sostituire Poincarè con il gruppo Conforme. 

Il teorema sembrerebbe spezzare le nostre speranze. D'altra parte, possiamo cercare una via d'uscita, una ipotesi del teorema alla quale possiamo rinuciare oppure modificare. In particolare ci poniamo la seguente domanda: se non esistono cariche vettoriali conservate, possiamo invece considerare cariche con indici \emph{spinoriali}? Questo significherebbe avere delle simmetrie che connetono stati di spin che differisce di \(1/2\), cioè tra bosoni e fermioni. Questo porta a modificare anche il concetto stesso di algebra: dovremmo considerare dei generatori fermionici anticommutanti. In effetti, si possono costruire queste teorie, e le algebre modificate in questo modo sono dette \emph{superalgebre di Lie}. Se queste contengono al loro interno l'algebra di Poincarè, si dicono algebre di \emph{supersimmetria} (oppure di superalgebre). 

\section{Definizione di superalgebra di Lie e primi esempi}

Cominciamo ricordando la definizione di algebra di Lie, \(\mathfrak{g}\): 

\begin{enumerate}
    \item \(\mathfrak{g}\) è uno spazio vettoriale su \(\mathbb{R}\) o \(\mathbb{C}\)
    \item Su questo spazio esiste una legge di composizione interna, bilineare e anticommutativa \([,]\) cioè 
    \begin{align}
        [, ] : \mathfrak{g} \times \mathfrak{g} \to \mathfrak{g}, \quad [\alpha A +\beta B, C] = \alpha [A, C] + \beta [B, C], \quad \alpha, \beta \in \mathbb{R}/\mathbb{C}, \quad [A, B] = -[B, A]
    \end{align}
    \item La legge soddisfa l'identità di Jacobi, cioè 
    \begin{align}
        [[A, B], C] + [[B, C], A] + [[C, A], B] = 0 
    \end{align}
\end{enumerate}

In particolare, se prendiamo \([A, B] = A\cdot B - B\cdot A\), le ultime due condizioni sono automaticamente verificate. 

Vogliamo ora generalizzare questa definizione. Siccome l'algebra al suo interno dovrà contenere sia bosoni sia fermioni, stabiliamo che i bosoni hanno grado 0 (mod. 2), e i fermioni hanno grado 1 (mod. 2). In questo modo possiamo richiedere che una superalgebra \(\mathfrak{S}\) sarà uno spazio vettoriale \emph{gradato}. In particolare uno spazio vettoriale \(V\) si dice gradato se \(V = V^0 \cup V^1\), e allo stesso tempo \(V = V^0 \cap V^1 = \left\{0\right\}\). Diamo quindi la definizione completa: 

\dfn{Superalgebra di Lie}{
    \(\mathfrak{S}\) si dice superalgebra di Lie se 
    \begin{enumerate}
        \item \(\mathfrak{S}\) è uno spazio vettoriale gradato, su \(\mathbb{R}\)/\(\mathbb{C}\). 
        \item Se \(\exists\) legge di composizione interna, bilineare e \emph{superanticommutativa}, cioè 
        \begin{align}
        ...
        \begin{align}
            \begin{cases}
                \text{due bosoni} \implies gr(A) = gr(B) = 0 \implies [A, B] = -[B, A] \\
                \text{fermione e bosone} \implies gr(A)gr(B) = 0 \implies [A, B] = -[B, A] \\
                \text{due fermioni} \implies gr(A)gr(B) = 1 \implies [A, B] = [B, A]
            \end{cases}
        \end{align}
        chiediamo inoltre che la legge sia additiva rispetto al grado, cioè che valga \(C = [A, B] \implies gr(C) = gr(A) + gr(B)\). 
        \item Valga super-Jacobi, cioè 
        \begin{align}
            (-)^{gr(A)gr(C)}[[A, B], C] + (-)^{gr(B)gr(A)}[[B, C], A] + (-)^{gr(C)gr(B)}[[C, A], B] = 0  
        \end{align}
    \end{enumerate}
}

Osserviamo che se prendiamo \([A, B] = A\cdot B - B\cdot A\) se almeno uno fra \(A, B\) è un bosone, e \([A, B] = A\cdot B + B\cdot A\) se entrambi sono fermioni, allora le ultime due condizioni sono automaticamente soddisfatte.  

Proprio come le algebre, il modo più semplice per descrivere una superalgebra è pensarla come matrici, oppure supermatrici, che agiscono su spazi vettoriali di rappresentazione gradati. 

Introduciamo uno spazio vettoriale \(V(m/n)\) gradato, su \(\mathbb{R}\) o \(\mathbb{C}\). I suoi elementi sono dati da 

\begin{align}
    \begin{pmatrix}
        x_1 \\ \ldots \\ x_m \\ y_1 \\ \ldots \\ y_n
    \end{pmatrix}, \quad \text{bosoni} = \begin{pmatrix}
        x_1 \\ \ldots \\ x_m \\ 0 \\ \ldots \\ 0
    \end{pmatrix}, \quad \text{fermioni} = \begin{pmatrix}
        0 \\ \ldots \\ 0 \\ y_1 \\ \ldots \\ y_m 
    \end{pmatrix}
\end{align}

Le trasformazioni di \(V\) in sè stesso, cioè gli endomorfismi, ereditano anche loro una struttura gradata: in particolare le trasformazioni bosoniche devono mandare bosoni in bosoni e fermioni in fermioni, mentre le trasformazioni fermioniche devono mandare bosoni in fermioni, e fermioni in bosoni. Segue che le trasformazioni bosoniche sono rappresentate da matrici diagonali a blocchi, mentre le trasformazioni fermioniche l'ooposto. 

Con la struttura di commutatore e anticommutatore ordinari, si definisce allora la superalgebra generale lineare, e si indica con \(\mathfrak{gl}(m/n)\)

Chiaramente a parte \(\mathfrak{gl}(n/m)\) si possono definire altri tipi di superalgebre, che sono sotto-algebre di \(\mathfrak{gl}(m/n)\) 

\ex{La superalgebra speciale lineare}{

    Si può definire ad esempio la superalgebra speciale lineare, che si indica con \(\mathfrak{sl}(m/n)\). Nel caso ordinario, si definiva 
    \begin{align}
        \mathfrak{sl}(m) = \left\{A \in \mathfrak{gl}(m) : \text{Tr}(A) = 0 \right\}
    \end{align}

    Nel caso delle superalgebre però, non basta imporre che la traccia si annulli. Ad esempio, considera le due trasformazioni bosoniche 
    ...
    \begin{align}
        \text{Tr}([M, N]) = \text{Tr}\begin{pmatrix}
            [A, A'] & 0 \\ 
            0 & [D, D']
        \end{pmatrix} = 0
    \end{align}

    Siccome la traccia del commutatore di due matrici è sempre nullo. La cosa funziona anche se \(M\) è un bosone ed \(N\) è un fermione: 
    \begin{align}
        M = \begin{pmatrix}
            A & 0 \\ 0 & D
        \end{pmatrix}, \quad N = \begin{pmatrix}
            0 & B \\ C & 0
        \end{pmatrix} \implies \text{Tr}([M, N]) = \text{Tr}\begin{pmatrix}
            0 & AB \\ DC & 0 
    ...
    I problemi si hanno quando ho due fermioni: 
    ...
    \begin{align}
        ...
        \end{pmatrix}, \quad N = \begin{pmatrix}
            0 & B' \\ C' & 0 
        \end{pmatrix} \implies \text{Tr}(\left\{M, N\right\}) \neq 0 
    \end{align}

    La via di uscita da questo problema è la definzione della supertraccia: 

    \begin{align}
        \text{Str}\begin{pmatrix}
            A & B \\ C & D 
        \end{pmatrix} = \text{Tr}(A) - \text{Tr}(D)
    \end{align}

    Definiamo così quindi l'algebra speciale lineare, 
    \begin{align}
        \mathfrak{sl}(m/n) = \left\{M \in \mathfrak{gl}(m/n) : \text{Str}(M) = 0 \right\}
    \end{align}

    Ora, una supertraccia contiene sempre un settore bosonico, che indichiamo con \({}^0\mathfrak{S}\), ed un settore fermionico, che chiamiamo \({}^1\mathfrak{S}\). Come è fatto il settore bosonico? le matrici in generale si possono scrivere in termini di una base \(M, N, I\), se 

    \begin{align}
        A = \begin{pmatrix}
            A & 0 \\ 0 & 0 
        \end{pmatrix}, \, \text{Tr}(A) = 0, \,  \quad B = \begin{pmatrix}
            0 & 0 \\ 0 & D
        \end{pmatrix}, \, \text{Tr}(B) = 0, \, \quad C = \begin{pmatrix}
            \alpha \mathbb{I} & 0 \\ 
            0 & -\frac{m}{n}\alpha \mathbb{I}
        \end{pmatrix}
    \end{align}

    Concludiamo che \({}^0 \mathfrak{sl}(m/n) = \mathfrak{sl}(m)\oplus \mathfrak{sl}(n)\oplus \mathfrak{u}(1)\), e quindi \(\dim({}^0 \mathfrak{sl}(m/n)) = m^2 -1 + n^2 -1 +1 = m^2 +n^2 -1\). 

    Con ragionamenti simili, si mostra che \({}^1\mathfrak{sl}(m/n)\) è la somma dei due blocchi off-diagonal, cioè \(\dim({}^1 \mathfrak{sl}(m/n)) = 2mn\). 
}

\ex{Superalgebra ortosimplettica \(\mathfrak{osp}(m/n)\)}{

    Possiamo dotare lo spazio \(V(m/n)\) con una forma bilineare, \((x, y) = x^T G y\), con 
    ...
    Se \(\mathbb{J}\) è la matrice simplettica. Si definisce allora la sottoalgebra ortosimplettica dell'algebra generale lineare nel seguente modo: 

    \begin{align}
        \mathfrak{osp}(m/n) = \left\{M \in \mathfrak{gl}(m/n) : (x, My) + (-1)^{gr(x)gr(y)}(Mx, y) = 0, \, \forall (x, y) \right\}
    \end{align}

    per studiare quindi la struttura della superalgebra partiamo dal suo settore bosonico. Quale è una base del settore bosonico? Separo i blocchi e indago: 

    \begin{enumerate}
        \item Blocco \(m\times m\): scrivo 
        \begin{align}
            M = \begin{pmatrix}
                A & 0 \\ 0 & 0 
            \end{pmatrix}
        \end{align}
        Quale deve essere la condizione affinché \(M \in \mathfrak{osp}(m/n)\)? siccome agisce su bosoni, deve rispettare \(x^T A y + (AX)^T y =0 \implies A + A^T = 0\), ovvero \(A \in \mathfrak{o}(m)\). 
        \item Blocco \(n\times n\): similmente, sia 
        \begin{align}
            N = \begin{pmatrix}
                0 & 0 \\
                0 & D 
            \end{pmatrix}
        \end{align}
        La condizione per cui \(N\) appartiene alla superalgebra è che valga \(\mathbb{J}D + D^T \mathbb{J} = 0\), ovvero \(D \in \mathfrak{sp}(n)\). 
    \end{enumerate}
    troviamo quindi che \({}^0 \mathfrak{osp}(m/n) = \mathfrak{o}(m) \oplus \mathfrak{sp}(n)\)
}

\section{L'algebra di supersimmetria}

Osserviamo che il settore bosonico \({}^0\mathfrak{S}\) di una superalgebra è sempre del tipo \(\mathfrak{g}_1 \oplus \mathfrak{g}_2\), con queste due algebre ordinarie. Questo è utile perché noi vogliamo scrivere delle algebre che contengano \(\mathcal{P} \oplus \mathfrak{g}\). La parte "nuova" quindi sarà tutta nel settore fermionico. 

In generale, indichiamo con \(B_i\) i generatori bosonici, e con \(Q_I\) i generatori fermionici. Capire come è fatto il settore fermionico vuol dire anche capire quali sono le relazioni di commutazione \(B_i, Q_I\). Dalla regola di additività del grado, si vede subito che sono dei generatori fermionici, e quindi dovrò espanderli in una base di generatori fermionici. In formule, scriveremo 

\begin{align}
    [Q_I, B_i] = (M_i)_{IJ}Q_J 
\end{align}

La matrice di coefficienti \((M_i)_{IJ}\) è vincolata dalla identità di Jacobi: 

\begin{align}
    0 = &[[Q_I, B_i], B_j] + [[B_j, Q_I], B_i] + [[B_i, B_j], Q_I] \notag \\
    &(M_i)_{IJ}[Q_J, B_j] - (M_j)_{IJ}[Q_J, B_i] + f_{ijk}[B_k, Q_I] = \notag \\
    &(M_i)_{IJ}(M_k){JK}Q_K - (M_j)_{IJ}(M_i)_{JK}Q_K + f_{ijk}(M_k)_{IK}Q_K 
\end{align}

Posso riscrivere l'eq. come 

\begin{align}
    [M_i, M_j]_{IK}Q_K = f_{ijk}(M_k)_{IK}Q_K
\end{align}

Che è un equazione interessante, perché mostra che le matrici \(M_i\) stanno in una rappresentazione della parte bosonica della superalgebra, con le \(Q_I\) che appartengono allo spazio di rappresentazione. Come già discusso, la parte bosonica per un'algebra di supersimmetria è \(\mathcal{P} \oplus \mathfrak{g}\): i generatori fermionici devono stare in una rappresentazione, cioè portare un doppio indice: uno della simmetria interna, e un'indice di lorentz (e.g. \(Q^i_\alpha\)). L'indice di lorentz sarà un indice di spin semi-intero, nel caso più semplice di spin 1/2. Poniamoci quindi proprio in questo caso (si dimostra che il caso generale è perfettamente analogo). Per quanto riguarda l'indice che corre sulla simmetria interna, per il momento lasciamolo libero di correre, \(i \in 1, \ldots, N\), così da non fare ipotesi sulla simmetria interna.  

Il nostro obbiettivo è quello di conoscere l'algebra di supersimmetria, cioè le seguenti parentesi: 
...
Si può mostrare che, senza perdere generalità, gli spinori \(Q_\alpha^i\) possono essere presi come spinori di Majorana, cioè che soddisfano \(Q_\alpha^i = C_{\alpha \beta}\bar{Q}^i_\beta\), con \(C\) la matrice di coniugazione della carica. 

Apriamo adesso una parentesi sulle convezioni che usiamo con gli spinori: 
